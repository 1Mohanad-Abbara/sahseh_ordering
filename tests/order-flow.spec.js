...
test("desktop renders the ordering menu, aligns hash sections, toggles cart, keeps data, and limits footer phone link", async ({ page }) => {
  ...
  await expect(page.locator(".neighborhood-option")).toHaveCount(27);
  ...
  expect(neighborhoodOptions).toContain("التوزيع الاجباري");
  expect(neighborhoodOptions).toContain("اوراس");
  await page.locator(".neighborhood-option", { hasText: "التوزيع الاجباري" }).click();
  await streetInput.fill("Homs 123, floor #5");
  await page.locator('input[name="deliveryCompany"][value="5g"]').check();
  await expect(page.locator(".checkout-final")).toContainText("260");
  await page.locator('input[name="deliveryCompany"][value="tbsher"]').check();
  await expect(page.locator(".checkout-final")).toContainText("250");
  await page.locator('input[name="deliveryCompany"][value="fast-delivery"]').check();
  await expect(page.locator(".checkout-final")).toContainText("330");
  await neighborhoodSelect.click();
