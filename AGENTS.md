...
- Delivery pricing is stored in a neighborhood-by-company matrix. `5G` and `Tbsher - تبشر` use provided neighborhood-specific prices for all 26 neighborhoods; `Fast Delivery` uses provided neighborhood-specific prices, with باب سباع still falling back to `300` for Fast Delivery until its real price is supplied. Delivery fees are internal calculation values and are never shown beside company names.
