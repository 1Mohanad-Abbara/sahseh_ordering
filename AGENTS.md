...
- Delivery service selection is required, labeled `خدمة التوصيل`, and must be exactly one of `5G`, `Tbsher - تبشر`, or `Fast Delivery`. Service availability is shown only in the final review message when known: `Tbsher - تبشر` is `10:00 AM - 12:00 PM`, `Fast Delivery` is `24 ساعة`, and `5G` is omitted until its time is known.
...
- Current smoke expectations are 13 categories, 13 sections, 104 products, 26 neighborhoods, delivery selection, final-review service availability, Tbsher and Fast Delivery pricing/WhatsApp routing, review/back/confirm flow, WhatsApp popup opening, success-popup return, and new-order reset.
